...
<script>
import { CompaniesAdd,CompaniesEdit } from "@/api/Companies.js";
export default {
  data() {
    return {
      isCom: true,
      ...
      formLabelWidth: "80px",
      rules: {
        eid: [{ required: true, message: "企业编号不能留空", trigger: "blur" }],
        name: [{ required: true, message: "企业名称不能留空", trigger: "blur" }]
      }
    };
  },
  methods: {
    sure() {
      this.$refs.form.validate(v => {
        if (v) {
          if (this.isCom) {
            CompaniesAdd(this.form).then(res => {
            ...
            CompaniesEdit(this.form).then(res => {
              //   window.console.log(res);
              if (res.data.code == 200) {
